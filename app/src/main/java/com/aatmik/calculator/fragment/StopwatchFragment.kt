...
import com.aatmik.calculator.databinding.FragmentStopwatchBinding
...
class StopwatchFragment : Fragment() {
        ...
        binding.apply {
            fabStartSW.setOnClickListener {
                ...
                if (timerState == TimerState.Stopped) {
                    fabStopSW.startAnimation(
                        AnimationUtils.loadAnimation(
                            requireContext(),
                            R.anim.fade_in_500
                        )
                    )
                    fabLap.startAnimation(
                        AnimationUtils.loadAnimation(
                            requireContext(),
                            R.anim.fade_in_500
                        )
    ...
    private fun updateStopwatchText() {
        ...
        if (updateTime >= 3600000) {
            hms = String.format(
                "%02d:%02d:%02d:%02d",
                (TimeUnit.MILLISECONDS.toHours(updateTime) - TimeUnit.DAYS.toHours(
                    TimeUnit.MILLISECONDS.toDays(
                        updateTime
                    )
                )),
                (TimeUnit.MILLISECONDS.toMinutes(updateTime) - TimeUnit.HOURS.toMinutes(
                    TimeUnit.MILLISECONDS.toHours(
                        updateTime
                    )
                )),
                (TimeUnit.MILLISECONDS.toSeconds(updateTime) - TimeUnit.MINUTES.toSeconds(
        ...
        if (binding.tvStopwatch != null) binding.tvStopwatch.text = hms
    }
