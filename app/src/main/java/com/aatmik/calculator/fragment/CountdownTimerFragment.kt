...
import com.aatmik.calculator.databinding.FragmentCountdownTimerBinding
...
class CountdownTimerFragment : Fragment() {
            ...
            sbHour.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(
                    ...
                    tvCountdownTimer.text = String.format(
                        "%02d:%02d:%02d",
                        progress,
                        sbProgressList["minute"],
                        sbProgressList["second"]
                    )
                }
    ...
    private fun startTimer() {
        ...
        timer = object : CountDownTimer(remainingTime, 1000) {
            ...
            override fun onFinish() {
                stopTimer()
                binding.tvCountdownTimer.text = "00:00:00"
            }
        }.start()
    ...
    fun updateCountDownText() {
        val hms = String.format(
            "%02d:%02d:%02d",
            (TimeUnit.MILLISECONDS.toHours(remainingTime) - TimeUnit.DAYS.toHours(
                TimeUnit.MILLISECONDS.toDays(
                    remainingTime
                )
            )),
            (TimeUnit.MILLISECONDS.toMinutes(remainingTime) - TimeUnit.HOURS.toMinutes(
                TimeUnit.MILLISECONDS.toHours(
                    remainingTime
                )
            )),
            (TimeUnit.MILLISECONDS.toSeconds(remainingTime) - TimeUnit.MINUTES.toSeconds(
        ...
        binding.tvCountdownTimer.text = hms
    }
